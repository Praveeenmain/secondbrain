...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchReflections = async () => {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch('https://secondbrainbackend-yvrs.onrender.com/api/reflections', {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const res = await fetch('https://secondbrainbackend-yvrs.onrender.com/api/chat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteReflection = async (id: string) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`https://secondbrainbackend-yvrs.onrender.com/api/reflections/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      const data = await res.json();
